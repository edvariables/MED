...
{// Source - https://stackoverflow.com/a/32810041
 ...
 // Retrieved 2026-09-03, License - CC BY-SA 4.0
    ...
    public class GifWriter : IDisposable
    {
        ...
        public static Bitmap ResizeGif(Bitmap image, Size size)
        {
            ...
            if (item == null || item.Value == null)
                ...
                frameDelay = (item.Value[0] + item.Value[1] * 256) * 10;

            if (frameCount > 1)
            {
                GIFFileInfo gifFileInfo=new();
                gifFileInfo.Initialize(image);

                MemoryStream writerStream = new MemoryStream();

                GifWriter gifWriter = new(writerStream, frameDelay);

                for (int frameIndex = 0; frameIndex < frameCount; frameIndex++)
                {
                    var hdl = image.GetHbitmap();
                    //image.SelectActiveFrame(FrameDimension.Time, frameIndex);
                    gifFileInfo.SetActiveFrame(frameIndex);

                    var resized = new Bitmap(gifFileInfo.image, size);

                    gifWriter.WriteFrame(resized, frameDelay);
                }

                gifWriter.Dispose();

                return (Bitmap)Bitmap.FromStream(writerStream);
            }
