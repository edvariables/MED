...
namespace libMotionDetection
{
    ...
    /// </summary>
    public class MotionDetectionWithMotionHistory : System.IDisposable
    {
        private static readonly ILogger logger = Log.Logger.ForContext (typeof (MotionDetectionWithMotionHistory));
        private readonly object _lock = new object ();
        private bool disposedValue = false;

        private readonly List<Rectangle> _motionZones = new List<Rectangle> ();

        /// <summary>
        /// Thread-safe read-only snapshot of the motion zones.
        /// To mutate this list, use AddMotionZone, RemoveMotionZone, or ClearMotionZones.
        /// </summary>
        public IReadOnlyList<Rectangle> MotionZones
        {
            get
            {
                lock (_lock) {
                    return _motionZones.ToArray ();
                }
            }
        }

        public void AddMotionZone (Rectangle zone)
        {
            lock (_lock) {
                _motionZones.Add (zone);
            }
        }

        public bool RemoveMotionZone (Rectangle zone)
        {
            lock (_lock) {
                return _motionZones.Remove (zone);
            }
        }
        ...
        private MotionComponent[] _motionComponents = new MotionComponent[] { };

        /// <summary>
        /// Thread-safe snapshot array of the motion components.
        /// </summary>
        public MotionComponent[] MotionComponents
        {
            get
            {
                lock (_lock) {
                    return (MotionComponent[]) _motionComponents.Clone ();
                }
            }
            private set
            {
                lock (_lock) {
                    _motionComponents = value;
                }
            }
        }
        ...
        public void GetFrameMotionComponents (Mat frame)
        {
            lock (_lock) {
                ...
                MotionComponents = GetFrameMotionComponents (History, ForgroundDetector, MotionForgroundMask,
                                                             ...
                                                             _motionZones, frame, Setting.CalculateMotionInfo);
            }
        }
        ...
        public void MotionDetectionDrawGraphics (Mat frame, LineSegment2D lineCrossing = new LineSegment2D ())
        {
            lock (_lock) {
                if (disposedValue) return;
                MotionDetectionDrawGraphics (_motionComponents, _motionZones, frame, lineCrossing);
            }
        }
        ...
        public void Reset (double mhiDuration = 2.0, double maxTimeDelta = 0.05, double minTimeDelta = 0.5)
        {
            ...
            lock (_lock) {
                // Dispose existing unmanaged resources to prevent leaks
                if (History != null) {
                    History.Dispose ();
                }
                if (ForgroundDetector != null) {
                    (ForgroundDetector as IDisposable)?.Dispose ();
                }
                if (MotionMask != null) {
                    MotionMask.Dispose ();
                }
                if (MotionForgroundMask != null) {
                    MotionForgroundMask.Dispose ();
                }
                ...
                _motionZones.Clear ();
                _motionComponents = new MotionComponent[] { };
        ...
        private Mat GetMotionImage (Mat motionMask, MotionHistory motionHistory)
        {
            ...
            using (ScalarArray sa = new ScalarArray (255.0 / maxVal))
        ...
        private Point[] GetPointsLine (Point p1, Point p2, int quantity)
        {
            ...
            int xdiff = p2.X - p1.X;
            int ydiff = p2.Y - p1.Y;
            int divisor = quantity - 1;

            for (int i = 0; i < quantity; i++) {
                double t = (double) i / divisor;
                int x = (int) Math.Round (p1.X + t * xdiff);
                int y = (int) Math.Round (p1.Y + t * ydiff);
        ...
        ~MotionDetectionWithMotionHistory ()
        {
